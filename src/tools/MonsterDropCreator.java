...
public class MonsterDropCreator {
	...
	private static boolean addFlagData = false; // There isn't any flag on my
												// source .. so.
	...
	private static int IncrementRate(int itemid, int times) {
		if (times == 0) {
			if (itemid == 1002357 || itemid == 1002926 || itemid == 1002927) { // Zakum
																				// Helmet,
																				// Targa,
																				// Scar
				return 999999;
	...
	private static int getChance(int id, int mobid, boolean boss) {
		switch (id / 10000) {
			...
			case 103: // Earring
				switch (id) {
					case 1032062: // Element Pierce - Neo Tokyo
						return 100; // 7 Slots earring with +2 to
									// str/dex/int/luk/ma
				}
				return 1000;
			case 105: // Overall
			...
			case 400:
				...
				switch (id / 1000) {
					case 4000: // ETC
					case 4001: // Story book, manon cry, orbis rock, eraser,
								// certificate
						return COMMON_ETC_RATE;
					case 4003: // Screw, Processed wood, Piece of Ice, Fairy
								// Wing, Stiff Feather, Soft Feather
						return 200000;
	...
	private static Map<Integer, List<Integer>> getDropsNotInMonsterBook() {
		...
		IndiviualMonsterDrop.add(2000004); // Elixir
		IndiviualMonsterDrop.add(2040813); // Cursed Scroll for Gloves for HP
											// 30%
		...
		IndiviualMonsterDrop.add(2040306); // Cursed Scroll for Earring for DEX
											// 70%
		IndiviualMonsterDrop.add(2043704); // Cursed Scroll for Wand for Magic
											// Att 70%
		IndiviualMonsterDrop.add(2044605); // Cursed Scroll for Crossbow for ATT
											// 30%
		...
		IndiviualMonsterDrop.add(2043705); // Cursed Scroll for Wand for Magic
											// Att 30%
		IndiviualMonsterDrop.add(2040716); // Cursed Scroll for Shoes for Speed
											// 30%
		IndiviualMonsterDrop.add(2040908); // Cursed Scroll for Shield for HP
											// 70%
		IndiviualMonsterDrop.add(2040510); // Cursed Scroll for Overall Armor
											// for DEF 70%
		...
		IndiviualMonsterDrop.add(2044404); // Cursed Scroll for Pole Arm for ATT
											// 70%
		IndiviualMonsterDrop.add(2044702); // Scroll for Claw for ATT 10%
		IndiviualMonsterDrop.add(2044305); // Cursed Scroll for Spear for ATT
											// 30^
